...
```
### WSL2 hardened (Telegram-only inbound, no host mounts)

This flow is for **WSL2 + Docker Desktop** when you want the gateway fully
contained (no host bind mounts), **no exposed ports**, and Telegram as the only
inbound surface. It uses named volumes and the hardened compose override.

**Files involved:**
- `docker-compose.hardened.yml` (override)
- `configs/moltbot.wsl-telegram.hardened.json5` (config template)

**1) Build the image**
```bash
docker build -t moltbot:local -f Dockerfile .
```
Optional (extra tools inside the container):
```bash
docker build \
  --build-arg "CLAWDBOT_DOCKER_APT_PACKAGES=git curl python3" \
  -t moltbot:local -f Dockerfile .
```

**2) Create named volumes**
```bash
docker volume create moltbot_state
docker volume create moltbot_workspace
```

**3) Seed the config into the state volume**
```bash
docker run --rm \
  -v moltbot_state:/data \
  -v "$(pwd)/configs/moltbot.wsl-telegram.hardened.json5:/tmp/moltbot.json5:ro" \
  alpine sh -lc "mkdir -p /data && cp /tmp/moltbot.json5 /data/moltbot.json"
```

**4) Set required env vars**
```bash
export TELEGRAM_BOT_TOKEN="replace-me"
export CLAWDBOT_GATEWAY_TOKEN="replace-me"
```

**5) Start the gateway (hardened)**
```bash
docker compose -f docker-compose.yml -f docker-compose.hardened.yml up -d moltbot-gateway
```

**6) Add Telegram allowlist + pairing (optional CLI)**
```bash
docker compose -f docker-compose.yml -f docker-compose.hardened.yml run --rm moltbot-cli channels add --channel telegram --token "$TELEGRAM_BOT_TOKEN"
```

Notes:
- No ports are published. The Control UI is **disabled** in the hardened config.
- All config/state/workspace live in Docker volumes (no host FS escape).
- If you later want the Control UI, remove `gateway.controlUi.enabled: false` and
  explicitly map the gateway port in a separate compose override.
...
## Security checklist (containerized gateway)

- [ ] No ports published unless explicitly needed.
- [ ] `gateway.bind: "loopback"` and `gateway.controlUi.enabled: false`.
- [ ] `discovery.mdns.mode: "off"` (no LAN discovery).
- [ ] Only required channels enabled (Telegram only).
- [ ] DM policy is `pairing` or allowlist.
- [ ] `tools.web.search` and `tools.web.fetch` disabled.
- [ ] `tools.elevated.enabled: false` unless explicitly needed.
- [ ] Config and workspace live in Docker volumes (no host bind mounts).
